...
import cv2
import numpy as np
# ... your existing vehicle detection code from main.py ...
import numpy as np
...
cap = cv2.VideoCapture("video/vehical.mp4")  #For Video
...
def run_detection():
    # Capture a single frame for demonstration purposes
    success, img = cap.read()

    # Process the frame using your vehicle detection logic
    results = model(imgRegion, stream=True)
    # ... (process detections, update counters, etc.) ...

    # Prepare response data (consider security implications)
    response_data = {
        'vehicle_count': len(totalCount1) + len(totalCount2) + len(totalCount3) + len(totalCount4),
        # Example: Include counts for specific lanes if appropriate
        # 'lane1_count': len(totalCount1),
        # ...
    }
